...
router.get('/getreport/:from/:to', async (req, res) => {
  ...
  WITH main AS(SELECT COUNT(DISTINCT orders.id) as orders_count, SUM(CASE WHEN order_chunks.part_id IS null 
    THEN 0 ELSE order_chunks.quantity END) as parts_sold_amount,  dist_sum(DISTINCT order_chunks.id, order_chunks.sell_price*order_chunks.quantity) as post_sale_all_value,
    SUM(normal_parts.price*order_chunks.quantity) as parts_value, SUM(comp_parts.price*computer_pieces.quantity) as computers_value,
  ...
  const byMonthInfo = `
SELECT DATE_TRUNC('month', orders.sell_date) as month, SUM(order_chunks.quantity) as items_amount, 
SUM(normal_parts.price*order_chunks.quantity)+ SUM(comp_parts.price*computer_pieces.quantity)
as items_value, 
	dist_sum(DISTINCT order_chunks.id, order_chunks.sell_price*order_chunks.quantity)
	-SUM(normal_parts.price*order_chunks.quantity)-SUM(comp_parts.price*computer_pieces.quantity) as profit
	FROM orders 
    JOIN order_chunks ON order_chunks.belonging_order_id = orders.id
    ...
    ORDER BY DATE_TRUNC('month', orders.sell_date)
    `
  const byWeekInfo = `
