...
router.get('/multisearch', async (req, res) => {
  ...
  let searchStrings = {
    ...
    WHERE parts.document_with_weights @@ to_tsquery('"${sQuery}":*') ORDER BY ts_rank(parts.document_with_weights, to_tsquery('"${sQuery}":*')) LIMIT 6`,
    ...
    GROUP BY computers.id ORDER BY ts_rank(computers.document_with_weights, to_tsquery('"${sQuery}":*')) desc LIMIT 6`,
    ...
    ORDER BY ts_rank(clients.document_with_weights, to_tsquery('"${sQuery}":*')) LIMIT 6`,
    ...
    ORDER BY ts_rank(suppliers.document_with_weights, to_tsquery('"${sQuery}":*')) LIMIT 6`,
    ...
    ORDER BY ts_rank(problems.document_with_weights, to_tsquery('"${sQuery}":*')) LIMIT 6`,
  };
